...
public class MainUI {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (input.equals("1")) {
				...
				if (result.equals("1"))
					System.out.println("[client] 회원가입 성공"); // Debug
				else
					System.out.println("[client] 회원가입 실패"); // Debug
	...
	class Receiver extends Thread { // 들어오는 데이터(채팅내용)
		...
		public void run() {
			...
			// e.printStackTrace();
			// }
			// System.out.println("[client [Receiver]] 서버로부터 받은 요청 :
			// "+receiveMsg.get("request")); //Debug
			// System.out.println("[client [Receiver]] 서버로부터 받은 full 메세지 : "+receiveMsg);
			// //Debug
			...
			while (true) {
				...
				try {
					...
					if (request.equals("msg")) {
						...
						game.init(receiveMsg);
					} else if (request.equals("start")) { // 방장 바뀜
						return;
					}
				} catch (ClassNotFoundException e) {
					e.printStackTrace();
